...
if __name__ == "__main__":
        ...
        def open_file(data,data_str,path):
            ...
            if file:
                ...
                try:
                    ...
                    size = 0
                    fail += 1
                    ret += "        Failed to get the size of {}\n".format(path)
                if fail == 0:
                    ret += "        {} was saved to {} ({} bytes)\n".format(data_str,path,size)
                    ret += "        {} contains {} entries.\n".format(data_str,len(data))
                file.close()
            else:
                ret += "        Failed to open {} at {}\n".format(data_str,path)
            print ret
            ...
            if os.path.exists(path):
                ...
                if file1:
                    try:
                        ...
                        ret += "        Loaded {} from {} ({} bytes)\n".format(data_str,path,size1)
                        ret += "        {} contains {} entries.\n".format(data_str,len(data))
                    except:
                        size1 = 0
                        ret += "        Failed to load {} from {}\n".format(data_str,path)
                else:
                    ret += "        Failed to open {}\n".format(path)
            else:
                ret += "        {} does not exist\n".format(path)
        ...
        def execute_cmd(c,index,graph, ranks, searches):
                if c == '1':
                    ...
                    if len(l) == 0:
                        ret = "        {} was not found in index".format(w)
                    else:
                        ret += "        '{}' appears in the following urls:\n".format(w)
                        for e in l:
                            ret += "            {}\n            score = {}\n".format(e[1],e[0])
                    searches.append(w)
                    is_trending = {}
